...
!
! Main program: p6hourly_to_1hourly
! Programmer: Yan Luo     Org: XXXX     Date: 2011-02-18
!
! Abstract: This program reads in 6-hour RFC QPE precipitation
!    and  disaggregates into hourly RFC precip
!
! Program history log:
!
! Input files: rfc_orig_T.grb for T=1,2,3,4,5,6
!
! Output files: adjusted_6hr.T.grb, for T=1,2,3,4,5,6
!
! Attributes:
!   Languate: Fortran 90
...
if (iargc() < 2) then
	print *, 'Usage: p6hourly_to_1hourly date1 date2'
	print *, 'where date(n) is the start date of the precip in the nth 1-hour slot'
...
!
!print *, ' - Determining hourly percentages...'
! Sum p6hourly precip
p6hourly_grid = 0
do t=1,tdim
...
!
!print *, ' - Determining hourly RFC precip...'
do t=1,tdim
 	where (temp_mask .and. p6hourly_rfc_scaled >= 0.001 .and. p6hourly_grid <  0.001) percent(:,:,t) = 1.0/6.0
...
!
!print *, ' - Writing hourly RFC precip to grib files:'
do t=1,tdim
	! Set file name
	write (file_name,fmt='(A,I1,A)') 'rfc_adjusted_1hr.',t,'.grb'
!	print *, file_name
	! Open grib file
