...
! Program history log:
!
! Input files: rfc_06h.1.grb, rfc_06h.2.grb, rfc_06h.3.grb, rfc_06h.4.grb
! 
! Output files: rfc_24h.grb
...
real, dimension(:,:), allocatable :: pcp_6hr, test
real, dimension(:,:), allocatable :: pcp_24hr
...
! Open grib file
call baopen(1,'rfc_06h_1.grb',stat)
if (stat .ne. 0) STOP 'Problem opening grib file rfc_06h_1.grb'
...
! Close grib file
call baclose(1,stat)
if (stat .ne. 0) STOP 'Problem closing grib file rfc_06h_1.grb'
...
!
allocate(temp_array(nx*ny),lb(nx*ny),pcp_6hr(nx,ny),pcp_24hr(nx,ny),&
bitmap(nx,ny,4),bitmap_final(nx,ny),missing(nx,ny),test(nx,ny))

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! READ IN ALL 6-HOUR PRECIP GRIDS
!
! Set some grib reading properties
jpds = -1 ! PDS input to getgb
jgds = -1 ! GDS input to getgb
...
! Loop over all 4 6-hr precip files
pcp_24hr = 0
do i=1,4
	! Set filename
	write (filename,'(A8,I1,A4)') 'rfc_06h_',i,'.grb'
	...
	pcp_6hr        = reshape(temp_array,(/nx,ny/))
!	where (pcp_6hr >= 0) pcp_24hr=pcp_24hr+pcp_6hr
 	where (pcp_6hr >= 0.01) pcp_24hr=pcp_24hr+pcp_6hr  !results same as previous statement
...
! Open grib file
call baopen(6,'rfc_24h.grb',stat)
...
temp_array = reshape(pcp_24hr,(/nx*ny/))
