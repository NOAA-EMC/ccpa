program COMP_ST4_ST2_1h_files
!###########################################################
!HISTORY:
!02/18/2011: Program created by Yan Luo for 3 horly composites
...
!09/02/2017: Program added by Yan Luo for 1 horly composites
!###########################################################

!$$$ MAIN PROGRAM DOCUMENTATION BLOCK
!
! Main program: COMP_ST4_ST2_1h_files
! Programmer: Yan Luo     Org: XXXX     Date: 2011-02-18
!
! Abstract: This program reads in hourly RFC QPE precipitation
!    grids from Stage II and Stage IV, and combines them into hourly precip
!
! Program history log:
!
! Input files: rfc2_01h.grb, rfc4_01h.grb, CCPA_CONUS_rfc_mask_hrap.grb 
! 
! Output files: rfc_1h.grb
...
real, dimension(:,:), allocatable :: pcp_1hr, test, rfc_mask
real, dimension(:,:), allocatable :: pcp2_1hr,pcp4_1hr
...
if (command_argument_count() < 4) then
	print *, 'Usage: COMP_ST4_ST2_1h_files yyyy mm dd hh'
...
!
allocate(temp_array(nx*ny),lb(nx*ny),pcp2_1hr(nx,ny),pcp4_1hr(nx,ny),&
bitmap(nx,ny,3),bitmap_final(nx,ny),missing(nx,ny),test(nx,ny),&
pcp_1hr(nx,ny),rfc_mask(nx,ny))
...
! Read 3-hr Stage IV precip file without data in CNRFC (rfc_mask=1) and NWRFC (rfc_mask=12)  
        pcp_1hr = 0
	! Set filename
	filename='rfc4_01h.grb'
        print *, 'read file:', filename
	! Open grib file
	call baopen(2,'rfc4_01h.grb',stat)
	if (stat .ne. 0) STOP 'Problem opening grib file'
	...
	! Reshape stream of data into 2D matrix
	pcp4_1hr        = reshape(temp_array,(/nx,ny/))
 	where ( rfc_mask .ne. 1.0 .and. rfc_mask .ne. 12.0 ) pcp_1hr=pcp_1hr + pcp4_1hr  !results same as previous statement
...
! Read 3-hr Stage II precip file with data in CNRFC and NWRFC only
	! Set filename
	filename='rfc2_01h.grb'
        print *, 'read file:', filename
	! Open grib file
	call baopen(3,'rfc2_01h.grb',stat)
	if (stat .ne. 0) STOP 'Problem opening grib file'
	...
	! Reshape stream of data into 2D matrix
	pcp2_1hr        = reshape(temp_array,(/nx,ny/))
! Combine 3-hr Stage II precip file with 3-hr Stage IV precip file
 	where ( rfc_mask == 1.0 .or. rfc_mask == 12.0 ) pcp_1hr= pcp_1hr + pcp2_1hr  !results same as previous statement
! DHOU: Tests show that the accuracy of the 6h files is 0.1mm, so 0.01 is used as threshold
...
temp_array = reshape(pcp_1hr,(/nx*ny/))
